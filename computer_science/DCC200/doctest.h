...
{ // NOLINT(*-avoid-c-arrays)
	...
	filloss<typename types::remove_reference<decltype(in)>::type>(stream,
								      in);
}
...
struct StringMaker
	: public detail::StringMakerBase<
		  ...
		  detail::is_pair<T>::value || detail::is_container<T>::value> {
};
...
template <typename T> String toString()
{
	...
	return ret.substr(beginPos + 1, ret.size() - beginPos -
...
#define DOCTEST_APPROX_PREFIX           \
	...
	friend typename std::enable_if< \
		std::is_constructible<double, T>::value, bool>::type
...
#define DOCTEST_DO_BINARY_EXPRESSION_COMPARISON(op, op_str, op_macro)         \
	...
	{                                                                     \
		...
		if (!res || doctest::getContextOptions()->success)            \
			return Result(res,                                    \
				      stringifyBinaryExpr(lhs, op_str, rhs)); \
		return Result(res);                                           \
	}                                                                     \
...
struct DOCTEST_INTERFACE ExpressionDecomposer {
	...
	{ // bitfields bind to universal ref but not const rvalue ref
		return Expression_lhs<const L &&>(
			static_cast<const L &&>(operand), m_at);
	}
...
namespace detail
{
template <typename T> int instantiationHelper(const T &) noexcept
{
	return 0;
}
...
#define DOCTEST_IMPLEMENT_FIXTURE(der, base, func, decorators)     \
	...
	DOCTEST_INLINE_NOINLINE void                               \
	der::f() // NOLINT(misc-definitions-in-headers)
...
#define DOCTEST_TEST_CASE_TEMPLATE_INSTANTIATE_IMPL(id, anon, ...)                                   \
	DOCTEST_GLOBAL_NO_WARNINGS(                                                                  \
		...
		doctest::detail::instantiationHelper(                                                \
			DOCTEST_CAT(id, ITERATOR) < __VA_ARGS__ >                                    \
			(__FILE__, __LINE__, 0)))
...
#define DOCTEST_TEST_SUITE(decorators)      \
	DOCTEST_TEST_SUITE_IMPL(decorators, \
				DOCTEST_ANONYMOUS(DOCTEST_ANON_SUITE_))

// for starting a testsuite block
#define DOCTEST_TEST_SUITE_BEGIN(decorators)                                 \
	DOCTEST_GLOBAL_NO_WARNINGS(                                          \
		DOCTEST_ANONYMOUS(                                           \
			DOCTEST_ANON_VAR_), /* NOLINT(cert-err58-cpp) */     \
	...
	{                                                                       \
		...
		DOCTEST_WRAP_IN_TRY(                                            \
			DOCTEST_RB.binary_assert<                               \
				doctest::detail::binaryAssertComparison::comp>( \
				__VA_ARGS__))                                   \
		DOCTEST_ASSERT_LOG_REACT_RETURN(DOCTEST_RB);                    \
	}                                                                       \
	DOCTEST_FUNC_SCOPE_END
...
#define DOCTEST_ASSERT_THROWS_AS(expr, assert_type, message, ...)                     \
	...
	{                                                                             \
		if (!doctest::getContextOptions()->no_throw) {                        \
			doctest::detail::ResultBuilder DOCTEST_RB(                    \
				doctest::assertType::assert_type, __FILE__,           \
				__LINE__, #expr, #__VA_ARGS__, message);              \
			try {                                                         \
				DOCTEST_CAST_TO_VOID(expr)                            \
			} catch (const typename doctest::detail::types::remove_const< \
				 typename doctest::detail::types::                    \
					 remove_reference<__VA_ARGS__>::type>::       \
					 type &) {                                    \
				DOCTEST_RB.translateException();                      \
...
#define DOCTEST_REGISTER_EXCEPTION_TRANSLATOR(signature) \
	...
	static inline doctest::String DOCTEST_ANONYMOUS( \
		DOCTEST_ANON_TRANSLATOR_)(signature)
...
#define DOCTEST_RELATIONAL_OP(name, op)          \
	template <typename L, typename R>        \
	bool name(const DOCTEST_REF_WRAP(L) lhs, \
		  const DOCTEST_REF_WRAP(R) rhs) \
	{                                        \
		return lhs op rhs;               \
	}
...
#define DOCTEST_LOCK_MUTEX(name)                             \
	const std::lock_guard<std::mutex> DOCTEST_ANONYMOUS( \
		DOCTEST_ANON_LOCK_)(name);
...
template <typename T> class MultiLaneAtomic {
	...
	CacheLineAlignedAtomic
		m_atomics[DOCTEST_MULTI_LANE_ATOMICS_THREAD_LANES];
...
bool ResultBuilder::log()
{
	if (m_at & assertType::is_throws) {
		...
		m_failed = !m_threw_as ||
			   !m_exception_string.check(m_exception);
	} else if (m_at & assertType::is_throws_as) {
		m_failed = !m_threw_as;
...
#define DOCTEST_INTERNAL_ERROR(msg)                                          \
	detail::throw_exception(std::logic_error(__FILE__ ":" DOCTEST_TOSTR( \
...
struct ConsoleReporter : public IReporter {
	...
	static const char *getSuccessOrFailString(bool success,
						  assertType::Enum at,
						  const char *success_str);

	static Color::Enum getSuccessOrFailColor(bool success,
						 assertType::Enum at);
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)                  \
	...
	DOCTEST_PARSE_AS_BOOL_OR_FLAG("case-sensitive", "cs", case_sensitive,
				      false);
	...
	DOCTEST_PARSE_AS_BOOL_OR_FLAG("force-colors", "fc", force_colors,
				      false);
	...
	DOCTEST_PARSE_STR_OPTION("strip-file-prefixes", "sfp",
				 strip_file_prefixes, "");
	...
	DOCTEST_PARSE_AS_BOOL_OR_FLAG("no-debug-output", "ndo", no_debug_output,
				      false);
	DOCTEST_PARSE_AS_BOOL_OR_FLAG("no-skipped-summary", "nss",
				      no_skipped_summary, false);
	DOCTEST_PARSE_AS_BOOL_OR_FLAG("no-time-in-output", "ntio",
				      no_time_in_output, false);
...
int Context::run()
{
	...
	for (auto &curr : getListeners())
		p->reporters_currently_used.insert(
			p->reporters_currently_used.begin(),
			curr.second(*g_cs));
	...
	const bool query_mode = p->count || p->list_test_cases ||
				p->list_test_suites;
	std::vector<const TestCaseData *> queryResults;

	if (!query_mode)
		DOCTEST_ITERATE_THROUGH_REPORTERS(test_run_start,
						  DOCTEST_EMPTY);
	...
	for (auto &curr : testArray) {
		...
		if (matchesAny(tc.m_test_suite, p->filters[3], false,
			       p->case_sensitive))
			skip_me = true;
		if (!matchesAny(tc.m_name, p->filters[4], true,
				p->case_sensitive))
			skip_me = true;
		if (matchesAny(tc.m_name, p->filters[5], false,
			       p->case_sensitive))
			skip_me = true;
		...
		if (p->list_test_suites) {
			if ((testSuitesPassingFilt.count(tc.m_test_suite) ==
			     0) &&
			    tc.m_test_suite[0] != '\0') {
				queryResults.push_back(&tc);
		...
		// execute the test if it passes all the filtering
		{
			...
			do { // NOLINT(cppcoreguidelines-avoid-do-while)
				...
				try {
					...
					DOCTEST_ITERATE_THROUGH_REPORTERS(
						...
						{ translateActiveException(),
						  false });
					p->failure_flags |=
						TestCaseFailureReason::Exception;
				}
#endif // DOCTEST_CONFIG_NO_EXCEPTIONS
				...
				const bool has_next_path =
					run_test ? p->traversal.advance() :
						   false;

				if (has_next_path && run_test)
					DOCTEST_ITERATE_THROUGH_REPORTERS(
						test_case_reenter, tc);
				if (!has_next_path)
					run_test = false;
			} while (run_test);
			...
			if (p->abort_after > 0 &&
			    p->numAssertsFailed >= p->abort_after)
				break;
		}
	}
...
bool operator==(double lhs, const Approx &rhs)
{
	...
	return std::fabs(lhs - rhs.m_value) <
	       rhs.m_epsilon *
		       (rhs.m_scale + std::max<double>(std::fabs(lhs),
						       std::fabs(rhs.m_value)));
}
	...
	if (getContextOptions()->no_path_in_filenames) {
		...
		for (String::size_type pos = 0U; pos < prefixes.size(); ++pos) {
			...
			if (prefix_size > longest_match) {
				...
				if (0 == std::strncmp(prefixes.c_str() +
							      prefix_start,
						      file, prefix_size)) {
...
const IContextScope *const *IReporter::get_active_contexts()
{
	return get_num_active_contexts() ? detail::g_infoContexts.data() :
					   nullptr;
}
...
void ConsoleReporter::test_run_end(const TestRunStats &p)
{
	...
	auto totwidth = static_cast<int>(std::ceil(
		log10(static_cast<double>(
			      std::max(p.numTestCasesPassingFilters,
	...
	auto failwidth = static_cast<int>(std::ceil(log10(
		static_cast<double>(
			std::max(p.numTestCasesFailed,
				 static_cast<unsigned>(p.numAssertsFailed))) +
		1)));
	const bool anythingFailed = p.numTestCasesFailed > 0 ||
				    p.numAssertsFailed > 0;
	s << Color::Cyan << "[doctest] " << Color::None
...
void ConsoleReporter::test_case_exception(const TestCaseException &e)
{
	...
	file_line_to_stream(tc->m_file.c_str(), static_cast<int>(tc->m_line),
			    " ");
	successOrFailColoredStringToStream(false,
					   e.is_crash ? assertType::is_require :
							assertType::is_check);
	s << Color::Red
...
void JUnitReporter::test_case_end(const CurrentTestCaseStats &st)
{
	...
	if (st.failure_flags & TestCaseFailureReason::Timeout) {
		...
		*stream << "Test case exceeded time limit of "
			<< std::setprecision(6) << std::fixed << tc->m_timeout;
	...
	if (st.failure_flags &
	    TestCaseFailureReason::ShouldHaveFailedButDidnt) {
		testCaseData.addError("should_fail",
				      "Should have failed, but didn't");
	} else if (st.failure_flags &
		   TestCaseFailureReason::DidntFailExactlyNumTimes) {
		auto *stream = detail::tlssPush();
		*stream << "Should have failed exactly "
			<< tc->m_expected_failures << " times, but didn't";
...
FatalConditionHandler::handleException(PEXCEPTION_POINTERS ExceptionInfo)
{
	...
	static bool execute = true;
	{
		...
		if (execute) {
			bool reported = false;
			for (size_t i = 0; i < DOCTEST_COUNTOF(signalDefs);
			     ++i) {
...
void String::copy(const String &other)
{
	if (other.isOnStack()) {
		...
		memcpy(allocate(other.data.size), other.data.ptr,
		       other.data.size);
	}
} // NOLINT(clang-analyzer-cplusplus.NewDeleteLeaks)
...
String &String::operator+=(const String &other)
{
	...
	if (isOnStack()) {
		if (total_size < len) {
			...
			memcpy(buf + my_old_size, other.c_str(),
			       other_size + 1);
			// NOLINTNEXTLINE(clang-analyzer-cplusplus.NewDeleteLeaks)
			...
			memcpy(data.ptr + my_old_size, other.c_str(),
			       other_size + 1);
		}
	} else {
...
String::size_type String::size() const
{
	if (isOnStack())
		return last -
		       (static_cast<size_type>(buf[last]) &
...
bool SubcaseSignature::operator==(const SubcaseSignature &other) const
{
	return m_line == other.m_line &&
	       std::strcmp(m_file, other.m_file) == 0 && m_name == other.m_name;
}
...
bool Subcase::checkFilters()
{
	if (g_cs->traversal.activeSubcaseDepth() <
		...
		if (!matchesAny(m_signature.m_name.c_str(), g_cs->filters[6],
				true, g_cs->case_sensitive))
			return true;
		if (matchesAny(m_signature.m_name.c_str(), g_cs->filters[7],
...
TestCase::TestCase(const TestCase &other)
	noexcept // NOLINT(bugprone-copy-constructor-init)
	: TestCaseData()
{
	*this = other;
}
...
{
	const size_t maxDepth = std::min(m_decisionPath.size(),
					 m_discoveredDecisionPath.size());
